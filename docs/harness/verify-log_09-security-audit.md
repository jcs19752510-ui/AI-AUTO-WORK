...
> 대상: `docs/harness/09-security-audit.md`의 "재작업 addendum — REQ-019(댓글) 보안검증" 절(append). 위 원본 절(1차/2차 검증, 재작업 라운드 2 검증)은 그대로 보존한다.

## 1차 검증 (작성자 관점 자가 재검토)
- 검증자(역할): `09-security-auditor`(자가 재검토)
- 일시: 2026-09-25
- 체크리스트
  - [x] 작업 지시가 명시한 7개 최소 점검 항목(인증/인가, 인젝션, 스팸/무차별남용, 개인정보/컴플라이언스, 의존성, 시크릿노출, IDOR/권한우회)을 전부 반영했는가 — §B-5 커버리지 표로 1:1 대조 확인, 누락 없음.
  - [x] 08단계 addendum이 인계한 갭(REQ-019 착수 이전 산출물이라는 점)을 실제로 메웠는가 — `comments` 앱 코드 전체(models/views/forms/urls/migrations/notifications/templates/JS)를 새로 정독하고, WU-01~10 결과(원 09단계 §1~§10, 재작업 라운드 2)는 반복 인용만 하고 재검증하지 않았음을 재확인.
  - [x] 하위 단계(06/07/08단계)의 PASS 주장을 그대로 인용만 하지 않고 필요한 부분은 독립적으로 재현했는가 — `webapp/.harness-tmp/venv_09c_repro/`(신규 venv, Python 3.13, 검증 후 삭제)를 만들어 `page_id` 비숫자 입력을 실제 `django.test.Client`로 제출해 미처리 `ValueError`를 직접 재현했다(§B-4-6 CSEC-17). 나머지 항목은 06/07/08단계가 이미 실측한 결과를 반복 실행하지 않고 코드 검토+상위 문서 대조로 독립 재확인했다(작업 지시 §Out-of-Scope와 일치).
  - [x] 20년차 보안담당자 관점에서 작업 지시가 명시하지 않은 공격 표면도 관례적으로 훑었는가 — `comments/views.py`를 처음부터 끝까지 재독하며 모든 사용자 제어 입력(`page_id`, `author_name`, `body`, `hp_field`, `slug`)의 처리 경로를 하나씩 추적했고, 그 결과 `page_id` 미검증(DEF-09-07)을 발견했다.
  - [x] 개인정보 파기절차·전송원칙이 실제 구현과 정확히 일치하는지 줄 단위로 대조했는가 — `notifications.py`의 발송 내용과 `legal/migrations/0003_...`의 방침 문구, `03-system-design.md §5.6` 파기절차 문단을 서로 대조해 DEF-09-05를 발견했다.
  - [x] 설계서 자체의 최신성(자기모순 여부)을 확인했는가 — §5.6 서두와 문서 헤더(v1.5)/§3.2-1을 대조해 DEF-09-06(설계서 갱신 누락)을 발견했다.
  - [x] Teardown(규칙K)이 실제로 완료됐는가 — `venv_09c_repro`/`repro_pageid.py` 삭제 후 `automation/harness-janitor.sh --check` 재실행으로 "잔여 임시 아티팩트 없음" 확인, `git status`가 세션 시작 전 스냅샷과 정확히 일치함을 확인.
- 발견된 결함 목록:
  1. DEF-09-05(Medium) — `mail_admins` 알림이 §5.6 파기절차 범위 밖의 개인정보 사본을 남김.
  2. DEF-09-06(Low) — 03-system-design.md §5.6 서두가 REQ-019 v1.5 승격 이전 상태로 남아있는 문서 자기모순.
  3. DEF-09-07(Medium) — `page_id` 미검증으로 인한 미처리 예외(500) 및 관리자 알림 채널 남용 가능성.
- 조치 내용: (수정 후 → v1) 위 3건을 `09-security-audit.md` §B-6에 재현 절차·심각도 판단 근거와 함께 반영했다. 감사자는 코드/설계서를 직접 수정하지 않는다는 원칙에 따라 조치는 "보고"로 한정했다(규칙F). Critical/High가 아니므로 §B-9 최종 판정에는 영향 없음(PASS 유지).

---

## 2차 검증 (독립 심사자 관점 — 역할전환 재검토)
> "내가 이 사이트를 노리는 공격자라면 새로 생긴 댓글 기능에서 어디를 먼저 찌를까"라는 관점으로, 1차 검증자가 놓쳤을 법한 공격 표면을 재검토했다.

- 검증자(역할): `09-security-auditor`(독립 심사자 관점, 역할전환)
- 일시: 2026-09-25
- 체크리스트
  - [x] 1차 검증에서 발견한 3건이 문서에 재현 가능한 형태(파일/행 번호/실행 결과)로 정확히 반영되었는가 — `09-security-audit.md` §B-4/§B-6를 재열람해 확인.
  - [x] 1차가 다루지 않은 각도가 남아 있는가 — 아래 5가지를 추가로 의심하고 §B-10에 반영했다: ① 본문 길이/제어문자 남용(추가 결함 아님, 폼 `max_length` 선행 방어 확인), ② `comment_write_page`의 slug로 다른 Page 타입을 노려 크로스 콘텐츠 댓글을 유발할 수 있는지(모델 한정 QuerySet으로 불가능 확인), ③ 봇넷 규모 공격 시 사전승인제가 여전히 최종 방어선으로 작동하는지(작동함, 공개 노출/XSS/유출로 이어지는 경로 없음), ④ DEF-09-07의 실제 악용 가치 재검토(인증우회/유출 불가, Medium이 적절함을 재확인), ⑤ 10단계 담당자가 추가 질문 없이 착수 가능한 문서 완성도.
  - [x] 심각도 판정이 자의적으로 축소되지 않았는가 — §B-6-1에서 "소규모 프로젝트라서 괜찮다"는 논리를 명시적으로 배제하고, 실제 노출 범위(운영자 자신의 메일함 vs 제3자)·전송구간 암호화 여부·인증우회 가능성 등 구체적 근거로만 Medium/Low를 확정했음을 재확인.
  - [x] 문서 품질(체크리스트 커버리지, 재현 가능성, 근거 명시)과 시스템 상태(PASS/FAIL) 두 층위를 혼동하지 않았는가 — 원 09단계 verify-log가 이미 확립한 구분을 그대로 유지, Medium/Low 결함이 있어도 Critical/High가 없으므로 시스템 상태는 PASS로 판정함을 §B-9에 명시.
  - [x] 규칙F(감사자는 코드를 수정하지 않는다) 원칙을 실제로 지켰는가 — 이번 addendum 작업 중 `webapp/` 코드 파일을 전혀 수정하지 않았음을 §B-7-4 `git status`(세션 시작 전 스냅샷과 완전 일치)로 재확인.
- 발견된 결함 목록:
  1. 없음 — 1차에서 발견한 DEF-09-05/06/07 외 신규 Critical/High/Medium/Low 결함 없음.
- 조치 내용: 신규 조치 없음(2차 검증에서 추가 결함 미발견, 이 addendum 절을 최종본으로 확정).

---

## 최종 판정 (REQ-019 addendum)
- [x] **PASS** (Critical/High 결함 0건, 최소 2회 검증 완료. Medium 2건/Low 1건은 기록으로 남기고 비차단) — 10단계(배포테스트)로 handoff 가능
- [ ] CONDITIONAL PASS
- [ ] FAIL

**이 addendum으로 `docs/harness/09-security-audit.md`는 REQ-019(댓글) 범위까지 확장된 보안검증을 완료했다. 문서 최상단 배너 + "재작업 addendum — REQ-019(댓글) 보안검증" 절 기준으로 최종 PASS이며, 10단계(배포테스트)로 handoff 가능하다.**

## 절차 흐름 (참고용 다이어그램)

```mermaid
flowchart TD
    A["1차 검증: 작성자 관점, 7개 필수 점검항목 + 자체발견 입력검증 확인"] --> B{결함 발견?}
    B -->|Yes, Medium 2건/Low 1건| C["조치: 재현절차+근거와 함께 §B-6 반영(코드는 수정하지 않음)"]
    C --> D["2차 검증: 독립 심사자 관점(역할전환), 5가지 추가 공격표면 재검토"]
    D --> E{Critical/High 신규 발견?}
    E -->|No| F["최종 판정: PASS(Medium/Low는 기록, 비차단)<br/>10단계로 handoff"]
```
