...
- 입력: `docs/harness/02-planning.md`(v1.2, PASS), `docs/harness/01-trend-analysis.md`(PASS), `docs/harness/decisions.md`(DEC-001~044), `docs/harness/traceability.md`, `docs/harness/feature-WU-01-integration-test.md`(FAIL, DEF-001 — v1.2 규칙F 재작업 트리거), `docs/harness/09-security-audit.md`(FAIL, DEF-09-01 — v1.3 규칙F 재작업 트리거), `docs/harness/10-deploy-test.md`(CONDITIONAL PASS, DEF-10-01 — v1.4 규칙F 재작업 트리거), `docs/harness/units/unit-09-note.md`, `docs/harness/units/unit-01-note.md`(§0-2, 라운드2), `webapp/config/urls.py`, `webapp/subscribers/admin.py`, `webapp/config/settings/production.py`
- 작성일: 2026-09-16 (최초) / 2026-09-16 (v1.2 규칙F 재작업) / 2026-09-17 (v1.3 규칙F 재작업) / 2026-09-25 (v1.4 규칙F 재작업) / 2026-09-25 (v1.5 신규기능 REQ-019)
- 버전: v1.5 (신규 기능 — 사용자 요청으로 REQ-019 댓글 기능을 v1 범위로 승격. **§3.1 ERD(COMMENT)/§3.2-1(신설)/§4(엔드포인트 2건) 추가, 나머지 섹션은 v1.4와 완전히 동일.** §10 변경이력 참고)
...
erDiagram
    ...
    BLOGPOSTPAGE ||--o{ COMMENT : has
    COMMENT {
        int id PK
        int page_id FK "BlogPostPage"
        string author_name "회원가입 없음 — 자유입력 표시명, 실명 검증 없음"
        text body "plain text, HTML 이스케이프 렌더링(§5.2-1)"
        string status "pending/approved/rejected (§3.2-1, 기본값 pending)"
        string source_ip_masked "마지막 옥텟 마스킹, NewsletterSubscriber와 동일 원칙"
        datetime created_at
    }
```
...
### 3.2-1 Comment [DEC-047 — v1.5 신규, REQ-019]

사용자가 배포 전 내부테스트 요청과 함께 댓글 기능 추가를 지시해(2026-09-25) v1 범위로 승격됐다(WU-11 전체가 아니라 REQ-019 단독 — 검색/소셜공유(REQ-018/020)는 여전히 Out-of-Scope, `traceability.md` 참고). 회원가입이 없으므로(DEC-009) 댓글도 **비회원 자유입력**이다.

- **모더레이션(사용자 결정)**: **사전 승인제** — `status` 기본값 `pending`이며, `pending`은 공개 화면에 노출되지 않는다. Moderators 그룹(및 superuser)만 `approved`/`rejected`로 전환할 수 있다(Wagtail Snippet 권한, Category와 동일 패턴). Editors는 발행 권한이 없는 것과 동일한 논리로 댓글 승인 권한도 없다(§5.1 "관리 작업은 통제 수준별로 분리" 원칙의 연장).
- **수집 항목(사용자 결정)**: `author_name`(자유입력 표시명)만 받는다 — **이메일은 받지 않는다**(수집최소화 원칙, §5.2). 계정이 없으므로 `author_name`은 실명 검증이 없는 임의 문자열이며, 동명이인/중복 사용을 막지 않는다(신뢰 불필요 — 승인제가 최종 방어선).
- **본문 렌더링**: `body`는 plain `TextField`다. Wagtail RichText/StreamField를 쓰지 않고 템플릿에서 Django 기본 auto-escape + `linebreaks` 필터로만 렌더링한다 — 방문자 입력에 HTML을 허용하지 않는다(§5.2 "방문자 입력이 이스케이프 없이 렌더링되는 경로 없음" 원칙을 댓글에도 동일 적용, XSS 표면 최소화). `mark_safe`/`|safe`를 이 경로에 쓰지 않는다.
- **스팸 방지**: `NewsletterSubscriber`(WU-07)와 동일한 2중 방어 — 허니팟 필드(`hp_field`, 값 있으면 200 위장 응답 후 미저장) + IP 기준 레이트리밋(LocMemCache, `comments` 앱 전용 카운터). 사전 승인제가 최종 방어선이므로 레이트리밋 임계값은 뉴스레터와 동일 수준(10분/5회)으로 시작하고, 스팸이 실제로 몰리면 운영자가 조정한다(과설계 방지 — 지금은 근거 없는 값을 더 엄격하게 추측하지 않는다).
- **개인정보**: `author_name`은 자유입력 표시명이라 반드시 실명이 아니며, `source_ip_masked`는 `NewsletterSubscriber`와 동일하게 마지막 옥텟만 마스킹해 저장한다. §5.6 개인정보처리방침에 "댓글 작성 시 표시명과 마스킹된 접속 IP를 수집한다"는 문장을 추가해야 한다(REQ-007 범위 재검토 — 원 설계 §1.2 각주가 이미 예견한 요구사항).
- **삭제**: 거부(`rejected`)된 댓글과 승인(`approved`) 후 문제가 된 댓글 모두 Moderators가 Wagtail 스니펫 어드민에서 하드 삭제할 수 있다(뉴스레터처럼 "삭제 전용, 수정 불가" 정책까지는 아니다 — 댓글은 `status` 전환이 핵심 운영 동작이므로 change 권한도 함께 부여, `author_name`/`body` 원문은 승인 판단을 위해 수정 불가로 제한할지는 5단계 구현 세부사항으로 위임 가능한 수준의 결정이라 여기서 확정하지 않는다).
- **신규 등록 알림과 삭제의 관계 [DEC-053, 09단계 DEF-09-05 대응, v1.6]**: 신규 댓글 등록 시 운영자에게 표시명+본문 전문을 담은 이메일이 발송된다(`comments/notifications.py`, DEC-049, 500 에러 알림과 동일한 `DJANGO_ADMIN_EMAIL` 채널 재사용). **이 이메일 사본은 위 하드 삭제의 영향을 받지 않는다** — DB 레코드를 지워도 운영자 메일함의 사본은 그대로 남는다. 이는 §5.6 파기절차가 원래 전제하지 않았던 별도의 개인정보 사본 경로이므로, 운영자가 인지해야 할 사실로 `11-ops-handoff-runbook.md`에 명시적으로 반영한다(코드 변경은 하지 않는다 — 운영자가 모더레이션 판단을 하려면 알림에 본문이 보이는 것 자체가 필요한 기능이며, 이메일 사본까지 자동 삭제하는 기능은 이번 요청 범위를 벗어나는 과설계로 판단).
- **앱 경계**: 새 `comments` 앱으로 분리한다(§1.2 "경계 원칙" — `subscribers`가 개인정보 수집을 이유로 별도 앱인 것과 동일하게, 댓글도 스팸/모더레이션이라는 독립된 운영 관심사이므로 `blog` 앱에 섞지 않는다). `blog` 앱은 게시물 하단에 댓글 목록/폼 자리만 포함하고(뉴스레터가 이미 그렇듯), 모델/뷰/모더레이션 로직은 전부 `comments` 앱 소유다.

**의도적으로 만들지 않은 것**: 결제/구독 플랜/광고 노출 로그(§0, DEC-005), 회원(REQ-026, Out-of-Scope — 댓글은 회원 없이 구현), 답글(threading, 요청 범위 밖 — 과설계 방지, 필요해지면 별도 규칙F 대상), 백업 이력 테이블(GitHub Actions 실행 로그 + R2 오브젝트 목록으로 충분히 감사 가능 — DB에 별도 로그 테이블을 두는 것은 과설계로 판단).
...
| POST | `/comments/submit/` | 댓글 작성(REQ-019, v1.5 신규) | 없음(CSRF 토큰 필수) | `page_id`(필수, 대상 `BlogPostPage`), `author_name`(필수), `body`(필수), `hp_field`(허니팟) | 200/302 — **항상 "검토 후 게시됩니다" 안내**(승인 전이므로 즉시 노출 안 됨을 명확히 고지, §3.2-1) | 400(형식 오류/대상 게시물 없음 또는 비공개), 429(레이트리밋), `hp_field` 값 있으면 200 위장 응답 후 미저장 |
| GET | `/comments/form-fragment/` | 댓글 작성 폼 조각(캐시 안 됨, 뉴스레터 폼 조각과 동일 패턴) | 없음 | `page_id`(쿼리파라미터) | 200, HTML(항상 새 CSRF 토큰) | 404(대상 게시물 없음/비공개) |
| GET | `/healthz` | 운영용 헬스체크 | 없음(내부용, 필요시 IP 제한 검토) | - | 200, `"ok"` — **DB 접속까지 확인하지 않는 얕은(shallow) 헬스체크**로 설계(Neon 콜드스타트 중 오탐으로 인한 불필요한 재시작/알림을 피하기 위함, 트레이드오프 명시). **HTTPS 강제 리다이렉트 예외 대상**(§5.5.2-1, v1.4 — Render 헬스체크 프로브가 `X-Forwarded-Proto` 헤더 없이 직접 접속해도 200을 반환해야 함) | - |
...
#### 5.5.2-1 `/healthz` HTTPS 강제 예외 [DEC-044 — v1.4 규칙F 재작업, DEF-10-01]

> **v1.4 재작업 배경**: 10단계 배포테스트(`10-deploy-test.md` TC-004)가 실제 컨테이너 기동으로 `/healthz`에 `X-Forwarded-Proto` 헤더 없이 접근하면 위 §5.5.2의 `SECURE_SSL_REDIRECT=True`에 의해 301이 반환됨을 실측했다. 이 동작 자체는 §5.5.2의 의도된 결과이지만, **§4(API 명세)가 `/healthz`를 "Render 헬스체크가 호출하는 내부용 얕은 헬스체크"로 정의(WU-08 소유)하면서도, Render의 실제 헬스체크 프로브가 TLS-종료 엣지를 거치지 않고 인스턴스에 직접 접속해 `X-Forwarded-Proto` 헤더 없이 요청할 가능성을 §5.5.2(WU-01 소유)가 전혀 고려하지 않았다** — 두 섹션이 서로를 참조하지 않은 설계 공백이 근본 원인이다(10단계 §6 DEF-10-01). 이 공백을 메우지 않으면 Render의 zero-downtime 배포 특성상 **최초 배포 자체가 헬스체크를 영원히 통과하지 못해 막힐 수 있다**(10단계 §8 리스크).

`/healthz`는 인증 없는 "ok" 텍스트만 반환하는 얕은 헬스체크로(§4), 응답에 민감정보가 전혀 없으므로 HTTPS 강제 리다이렉트 예외로 두어도 보안 손실이 없다. 아래를 §5.5.2 설정에 추가한다(순서상 §5.5.2 코드 블록 바로 다음에 위치):

```python
# Render의 헬스체크 프로브가 X-Forwarded-Proto 헤더 없이 인스턴스에 직접
# 접속할 가능성이 있다(10단계 DEF-10-01, TC-004 실측). /healthz는 민감정보
# 없는 "ok" 응답뿐이므로(§4, 내부용 분류) HTTPS 강제 리다이렉트 예외로 두어도
# 보안 손실이 없고, Render의 실제 헤더 처리 방식과 무관하게 최초 배포의
# 헬스체크가 항상 통과하도록 보장한다. core/urls.py의 path("healthz", ...)와
# 정확히 일치해야 한다(request.path.lstrip("/") 기준으로 매칭됨).
SECURE_REDIRECT_EXEMPT = [r"^healthz$"]
```

- **범위**: 정규식이 `healthz` 경로 단 하나만 정확히 매칭하므로(`^healthz$`, 접두사 매칭 아님), 다른 어떤 경로도 이 예외의 영향을 받지 않는다 — WU-01(05단계) 재작업 라운드2가 `core.tests.HealthzHttpsRedirectExemptTests`로 예외 경로(200)와 비예외 경로(`/robots.txt`, 여전히 301)를 대조 실측했다.
- **§4와의 상호 참조**: 이번 재작업으로 §4의 `/healthz` 행과 이 §5.5.2-1이 서로를 명시적으로 참조하게 되어, 앞으로 §4나 §5.5가 각각 독립적으로 수정되더라도 이 경계 조건이 누락되지 않도록 했다(재발 방지).
...
이 서비스가 실제로 수집하는 개인정보는 **뉴스레터 구독 이메일**, **댓글 작성 시 표시명(v1.5, §3.2-1 — 이메일은 수집하지 않음)**, **웹 서버 접속 로그(IP 등, 플랫폼이 자동 생성)** 세 가지다(회원가입 없음, REQ-026 Out-of-Scope는 유효 — REQ-019는 v1.5부터 §3.2-1로 구현됨, **[2026-09-25 정정, DEF-09-06]** 이 문장이 v1.5 갱신 시 함께 수정되지 않아 자기모순이었던 것을 09단계 보안검증이 발견해 바로잡음).
...
| 11 | **(v1.4 신규, Resolved) `/healthz` HTTPS 리다이렉트 예외 설계 공백** | §5.5.2-1이 이번 재작업으로 신설되어 해소됨. 코드(`SECURE_REDIRECT_EXEMPT`)는 05단계(WU-01) 라운드1에서 이미 반영되어 있었고, 이번 v1.4는 그 결정의 설계 근거를 소급 기록하고 §4↔§5.5.2 상호참조를 신설해 재발을 방지한 것이다(DEC-044). 05단계(WU-01) 라운드2가 회귀 테스트(`HealthzHttpsRedirectExemptTests`)로 실측 확인했으며, 10단계 `10-deploy-test.md` CONDITIONAL PASS 조건 1번 해소로 이어진다. |

| 2026-09-25 | v1.4 → v1.5(신규 기능) | **REQ-019(댓글) v1 범위 승격**: ① §3.1 ERD에 `COMMENT` 엔티티 신설. ② §3.2-1 신설 — 사전 승인제(사용자 결정), 표시명만 수집(이메일 미수집, 사용자 결정), plain text 본문+이스케이프 렌더링(XSS 방지), 허니팟+레이트리밋 스팸 방지, `comments` 앱으로 격리. ③ §4에 `POST /comments/submit/`, `GET /comments/form-fragment/` 신설. ④ §9 REQ-019 매핑 갱신. ⑤ 문서 헤더 버전 정보 갱신 | **사용자 요청(신규 기능).** 사용자가 배포 전 내부테스트를 요청하며 "글쓰기/관리/수정/삭제/댓글"을 언급했고, 댓글이 이 프로젝트에 없음을 확인·보고한 뒤(DEC-046) 사용자가 "댓글 기능 추가해 주세요"로 명시적으로 결정했다(DEC-047 예정). 모더레이션 방식/수집 항목 2가지는 규칙A에 따라 질문해 확정받았다(사전승인제, 표시명만 수집). 내부검증(규칙B) 최소 2회를 이번 §3.1/§3.2-1/§4 신설분에 대해 수행 — `verify-log_03-system-design.md` "재작업 라운드 4" 참고 |

**규칙 A 재확인**: 위 11개 항목은 모두 (a) 이미 상위 단계에서 답이 정해졌거나(1번), (b) 기술적으로 되돌리기 쉬운 확인 사항이거나(6, 7번), (c) 법률 자문이 필요하지만 "서비스 아키텍처 설계"의 두 갈래 선택 문제는 아닌 것(8번), (d) 이미 이번 재작업에서 옵션 비교·코드 확인을 거쳐 자체판단으로 확정된 것(10, 11번, DEC-040/DEC-044)으로 분류된다. **이번 단계에서 사용자에게 별도로 질문해야 할, 결과가 실질적으로 달라지는 두 갈래 이상의 미해결 쟁점은 없다.**
...
| REQ-011 | §2.4, §6.3, §5.5.2-1(v1.4: healthz HTTPS 리다이렉트 예외) |
| REQ-012 | §6.3, §7.2, §7.4 |
...
| REQ-019 | **v1.5: §3.1 ERD(COMMENT)/§3.2-1(모델·모더레이션·스팸방지 설계)/§4(`/comments/*` 엔드포인트)로 구현 반영. §5.6(개인정보처리방침) 갱신 필요 — 11단계/legal 마이그레이션에서 반영** |
...
| 2026-09-25 | v1.3 → v1.4(규칙F 재작업) | **§5.5.2-1 신설(다른 섹션은 v1.3과 동일, §4/§9 표는 상호참조만 추가)**: ① `/healthz`를 `SECURE_REDIRECT_EXEMPT`로 HTTPS 강제 리다이렉트에서 예외 처리하는 설계 결정을 정식 반영(코드 자체는 이미 `production.py`에 반영되어 있었으나 설계서에 근거가 없던 공백을 소급 정리). ② §4 `/healthz` 행에 §5.5.2-1 상호참조 추가. ③ §9 REQ-011 매핑에 §5.5.2-1 추가. ④ 문서 헤더 버전 정보 갱신 | **규칙F(피드백 루프) 재작업.** 10단계 배포테스트(`10-deploy-test.md` TC-004)가 CONDITIONAL PASS 조건 1번으로 지목한 **DEF-10-01(High)**의 근본 원인을 "§4(healthz 계약, WU-08 소유)와 §5.5.2(HTTPS 강제, WU-01 소유)가 서로를 참조하지 않은 설계 공백(3단계)"으로 지목해 3단계부터 재작업을 요구함. 내부검증(규칙B) 최소 2회를 이번 §5.5.2-1 신설분에 대해 재수행 — `verify-log_03-system-design.md` "재작업 라운드 3" 참고. DEC-044(decisions.md)로 이번 라운드(DEF-10-01/DEF-10-03 통합) 근거를 기록. 05(WU-01, 회귀테스트 신설)→06→07→10단계 재확인이 이어진다(§8 항목 10 계열과 동일 원칙, 이번엔 코드 변경이 아니라 문서-코드 정합성 회복이므로 8/9 재실행은 불필요 — 코드 자체가 8/9단계 검증 당시 이미 동일했음, 아래 §8 항목 11 참고) |
