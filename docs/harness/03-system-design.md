...
- 입력: `docs/harness/02-planning.md`(v1.2, PASS), `docs/harness/01-trend-analysis.md`(PASS), `docs/harness/decisions.md`(DEC-001~014), `docs/harness/traceability.md`, `docs/harness/feature-WU-01-integration-test.md`(FAIL, DEF-001 — 규칙F 재작업 트리거 원본)
- 작성일: 2026-09-16 (최초) / 2026-09-16 (v1.2 규칙F 재작업)
- 버전: v1.2 (규칙F 재작업 — 07단계 통합테스트 FAIL(DEF-001, Critical) 근본원인 대응. §5.5만 보완, 나머지 섹션은 v1.1과 동일. §10 변경이력 참고)
...
| 로그인 무차별대입 방어 | django-axes 또는 django-ratelimit | 대체로 MIT/BSD 계열로 알려짐 | **확인 필요** — 5단계에서 실제 패키지 선정 시 라이선스 원문 재확인 |
...
**REQ-005 구조화 데이터(JSON-LD) 구현 방침**: `BlogPostPage` 상세 템플릿(`/blog/<slug>/`)에 `schema.org/BlogPosting` JSON-LD를 임베드한다. 필드 매핑: `headline`←`title`, `datePublished`←`first_published_at`, `dateModified`←`last_published_at`, `image`←`featured_image`의 R2 공개 URL, `author`←`owner`(Django User)의 표시 이름 또는 `SiteSettings.site_title`. 이 값들은 전부 신뢰된 운영자 입력 또는 시스템 필드이므로(외부 사용자 입력을 직접 삽입하지 않음) 별도 새니타이즈 라이브러리 없이 Django 템플릿의 기본 auto-escape만으로 충분하다(§5.2 XSS 방지 원칙과 일관). REQ-017(AI 검색 대응)의 질문-답변형 콘텐츠(FAQ StreamField 블록)도 같은 방식으로 `FAQPage` JSON-LD를 선택적으로 추가할 수 있도록 템플릿 블록을 분리해 둔다.

---

## 5. 보안 설계 원칙

### 5.1 인증/인가 모델 [DEC-009]
...
- **관리자 어드민 경로 변경**: `/admin/` → `/cms-admin/`(예시)로 변경해 자동화된 무차별 대입 시도 노출을 줄인다.
- **비밀번호 정책**: Django 기본 `AUTH_PASSWORD_VALIDATORS`(최소 길이, 일반 비밀번호 사전 차단 등) 활성화.
- **로그인 무차별대입 방어**: IP/계정 기준 레이트리밋(§2.6, 라이선스 확인 후 5단계에서 도입). 운영자가 1인/소규모라는 점을 감안해도, 어드민 경로가 공개 인터넷에 노출되는 이상 이 방어는 Should가 아니라 **필수**로 취급한다.
...
- **파기 절차**: 탈퇴 요청 접수 시 운영자가 Wagtail 어드민(또는 Django admin)에서 해당 `NewsletterSubscriber` 레코드를 하드 삭제한다(§3.2에서 명시한 대로 `status` 필드 토글이 아니라 실제 행 삭제). R2 백업(`backup-private`, pg_dump)에는 삭제 시점 이후의 스냅샷부터 반영되며, Lifecycle 정책(14일 자동 만료, §7.5)에 따라 이전 백업도 일정 기간 후 자동 파기된다 — 이 지연(최대 14일)을 방침에 명시할 것을 권고한다.
...
**규칙 A 재확인**: 위 8개 항목은 모두 (a) 이미 상위 단계에서 답이 정해졌거나(1번), (b) 기술적으로 되돌리기 쉬운 확인 사항이거나(6, 7번), (c) 법률 자문이 필요하지만 "서비스 아키텍처 설계"의 두 갈래 선택 문제는 아닌 것(8번)으로 분류된다. **이번 단계에서 사용자에게 별도로 질문해야 할, 결과가 실질적으로 달라지는 두 갈래 이상의 미해결 쟁점은 없다.**
...
- django-storages 라이선스: https://github.com/jschneier/django-storages (LICENSE 파일)
