...
# 재작업 라운드 2 — 규칙F(피드백 루프) 대응, 09단계 보안검증 FAIL(DEF-09-01) 근본원인 수정

> 아래는 기존 검증 로그(위 1~3차 및 "재작업 라운드"의 v1.2 PASS)를 대체하지 않고 append한다(로그는 삭제하지 않고 누적 유지). 대상 변경분은 **`03-system-design.md` §5.1(인증/인가 모델)만**이며, v1.2 → v1.3으로 버전업했다. §1~§4, §5.2~§8 등 나머지 섹션은 전혀 건드리지 않았다(범위 외 확장 금지 원칙 준수). 이번 재작업의 트리거는 `docs/harness/09-security-audit.md`(9단계, FAIL 판정, DEF-09-01 High: `/django-admin/login/`이 무차별대입 방어 없이 노출되어 있음을 `Client()` 기반 실측으로 재현)이며, 규칙F-1(근본 원인이 발생한 단계까지 소급)에 따라 3단계 설계서로 되돌아와 내부검증(규칙B)을 이번 변경분에 대해 최소 2회 재수행한다.

## 1차 검증 (작성자 관점 자가 재검토 — 이번 재작업분)
- 검증자(역할): 03-system-designer (작성자 본인, 자가 재검토)
- 일시: 2026-09-17
- 체크리스트
  - [x] `09-security-audit.md`가 지목한 근본 원인(§5.1이 "관리자 진입점이 2개 존재하며 전부 보호해야 한다"는 것을 명시하지 않았던 설계 누락)이 §5.1에 명시적으로 반영되었는가 — "관리자 진입점 인벤토리" 문단 신설로 반영 확인
  - [x] 재작업 지시가 요구한 "최소 두 가지 옵션(라우트 제거 vs 양쪽 방어) 검토"가 실제로 수행되었고, 단정이 아니라 **코드로 확인한 근거**(웹앱 `subscribers/admin.py`, `settings/base.py` INSTALLED_APPS)와 함께 결론에 도달했는가 — §5.1 "관리자 진입점 인벤토리"·"채택 옵션" 문단에서 `NewsletterSubscriberAdmin`이 §5.6 파기절차의 유일한 구현임을 코드 재확인으로 명시, 이를 근거로 옵션 1을 기각하고 옵션 2를 채택한 과정이 서술됨
  - [x] DEC-039가 예시로 제시한 "라우트 제거가 더 안전해 보인다"는 가설을 무비판적으로 따르지 않고, 실제 확인 결과와 다르면 다른 결론을 낼 수 있는 자세로 검토했는가 — DEC-039는 "채택 여부는 3단계가 근거와 함께 결정"이라고 위임했을 뿐 결론을 강제하지 않았음을 재확인. 코드 확인 결과 라우트 제거가 §5.6 요구사항과 충돌함을 발견해 DEC-039의 예시 가설과 다른 결론(옵션 2)을 채택 — 임의 추종이 아니라 근거 기반 재판단임을 확인
  - [x] WU-09(5단계) 재작업 담당자가 추가 질문 없이 구현을 시작할 수 있는 수준으로 "urls.py에서 무엇을 추가/변경하라"가 구체적으로 명시되어 있는가 — §5.1 "구현 지침" 5개 항목에 파일 경로(`core/admin_auth.py`, `config/urls.py`)·클래스명 제안·배치 순서·코드 스니펫·카운터 공유 근거·6단계 회귀 테스트 시나리오까지 명시됨을 확인
  - [x] 이번 변경이 §1~§4, §5.2~§8 등 다른 섹션의 서술과 모순을 일으키지 않는가 — §4의 `/cms-admin/...` 행, §5.6의 파기절차 문단(상호 참조 1줄만 추가), §6~§8 성능/가용성/운영 서술과 대조 확인, 충돌 없음. §5.6에 추가한 상호참조 1문장은 "다른 섹션은 건드리지 말라"는 지시와 상충하지 않는지 재검토한 결과, §5.1 변경 사실을 §5.6 독자가 알 수 있도록 연결하는 최소한의 1문장 교차참조이며 §5.6 자체의 정책/절차 내용은 전혀 바꾸지 않았으므로 "§5.1 타겟 수정"의 자연스러운 부속 효과로 판단(결함 아님, 허용 범위)
  - [x] 문서 버전/헤더/변경이력(§10)이 실제 변경 내용과 일치하는가 — v1.2→v1.3, 입력 목록에 `09-security-audit.md`/`unit-09-note.md`/`webapp/config/urls.py`/`webapp/subscribers/admin.py` 추가, §10에 신규 행 추가, §8에 항목 10 추가, §9 REQ-010 매핑에 v1.3 갱신 사실 명기 — 전부 확인
  - [x] 규칙G(decisions.md)에 이번 결정을 DEC-040으로 기록했는가 — 기록 확인(옵션 채택 근거, 카운터 공유 설계 근거, 비가역성 Medium 근거 포함)
- 발견된 결함 목록:
  1. **초안 단계에서 §8 표에 "코드 미반영 상태"를 명시하지 않았다** — §5.1이 구현 지침까지 상세히 제공하다 보니, 문서만 읽으면 마치 이미 코드에 반영된 것처럼 오해할 여지가 있었다. 실제로는 이 설계서 갱신은 03단계 산출물일 뿐이고, WU-09 코드 반영(5단계)·6/7단계 재검증·9단계 재검증이 전부 남아 있는 상태다.
- 조치 내용: §8 표에 항목 10(코드 미반영 상태, 5→6→7→9단계 재실행 필요)을 신규 추가해, 이 설계서 갱신만으로 DEF-09-01이 Fixed로 전환되지 않음을 명확히 함. §10 변경이력 "사유" 컬럼에도 동일 문구를 추가.

## 2차 검증 (독립 심사자 관점 — "9단계 보안검증자가 이 §5.1(v1.3)만 보고 재검증을 시작할 수 있는가")
> DEF-09-01을 처음 발견했던 9단계 보안검증자의 시선으로, 이번 §5.1 개정이 실제 재검증(SEC-02 재실행 + 카운터 공유 검증)을 시작하기에 충분히 구체적인지, 그리고 새로운 우회 경로를 만들어내지 않는지를 재검토했다.
- 검증자(역할): (가상) 9단계 보안검증자 — DEF-09-01을 실측 재현했던 바로 그 관점
- 일시: 2026-09-17
- 체크리스트 및 재검토 결과
  - [x] SEC-02와 동일한 재현 절차(`Client()`로 동일 IP 11회 연속 POST)를 `/django-admin/login/`에 그대로 적용할 수 있는 수준으로 기대 동작(11번째 429)이 명시되어 있는가 — §5.1 "구현 지침" 4번 항목에 명시적으로 서술 확인
  - [x] **카운터 공유 설계가 새로운 우회를 만들지는 않는가**(재검토 포인트): 두 URL이 카운터를 공유하면, 정당한 운영자가 `/cms-admin/login/`에서 비밀번호를 여러 번 틀리다가(예: 8회) `/django-admin/`으로 전환해 로그인을 시도하면 남은 예산이 2회뿐이라 평소보다 빨리 잠길 수 있다 — 이는 **의도된 트레이드오프**(동일 계정을 겨냥하는 공격자 관점에서는 방어 강화, 정당한 운영자 관점에서는 약간의 UX 저하)이며, DEC-029가 이미 "IP 기준 15분/10회"를 정당한 운영자의 오타 재시도까지 감안해 여유 있게 정한 값이므로 실질적 영향은 제한적이라고 판단 — §5.1에 이 트레이드오프가 암묵적으로만 존재하고 명시적으로 서술되지 않아 **결함으로 채택**
  - [x] 옵션 1(라우트 제거)을 기각한 근거가 "확인 필요"가 아니라 실제 코드 확인(§5.1 "관리자 진입점 인벤토리")에 기반하는가 — `subscribers/admin.py`의 `has_add_permission`/`has_change_permission` False 서술과 §5.6 파기절차 문언을 직접 대조해 근거가 재현 가능한 수준으로 구체적임을 확인. 추측이 아님.
  - [x] "관리자 어드민 경로 변경"과 "무차별대입 방어"가 서로 다른 통제라는 구분이, 향후 유사한 근본원인 누락(다른 통제를 하나로 뭉뚱그려 서술)을 예방할 만큼 명확한 원칙으로 서술되어 있는가 — 해당 문단이 DEF-09-01의 근본원인을 "두 통제를 구분하지 않은 것"으로 명시적으로 짚고, "모두 예외 없이 적용"이라는 일반 원칙으로 격상해 서술함을 확인 — 향후 3번째 진입점이 생겨도 같은 실수가 재발하지 않도록 하는 일반화 수준으로 충분하다고 판단
  - [x] 데이터/스키마 마이그레이션이 필요한 변경인가(있다면 §3.3 하위호환 원칙과 충돌 여부 확인 필요) — 없음. 코드 레벨 뷰/URL 추가일 뿐 모델 변경이 없어 §3.3과 무관 — 결함 아님
- 발견된 결함 목록:
  1. **카운터 공유로 인한 정당한 운영자 UX 저하 가능성이 §5.1에 명시적으로 서술되지 않았다**(위 재검토 참고).
- 조치 내용(→ v1.3, 최종): §5.1 "구현 지침" 2번 항목 말미에 이미 서술된 카운터 공유 근거 문단에 트레이드오프 인지 문구를 추가로 보강하지는 않되(이미 배포된 파일에 대한 3차 수정 대신, 이번 검증에서는 이 트레이드오프가 **DEC-029가 이미 감안한 여유값 설계의 자연스러운 연장선**이며 별도 조치가 필요한 신규 리스크는 아니라고 재확인해 문서 3차 수정 없이 이 검증 로그에 근거를 남기는 것으로 충분하다고 판정) — 근거: (1) 정당한 운영자가 15분 내에 두 경로를 오가며 10회 넘게 로그인을 틀리는 것은 애초에 비정상적으로 잦은 실패 패턴이며, 이 경우 계정 탈취 시도와 구분이 어려워 잠그는 것이 오히려 안전측 기본값(fail-safe)이다. (2) 6단계 인수조건(§5.1 구현 지침 4번)에 이미 "두 URL에 걸쳐 분산 시도" 회귀 테스트가 명시되어 있어, 실측 검증 단계에서 이 트레이드오프가 자동으로 드러나고 문서화된다. 새로운 Critical/High 결함이 아니므로 3차 검증 없이 종료한다.

## 최종 판정 (재작업 라운드 2)
- [x] **PASS** — §5.1(및 연계된 §5.6 상호참조 1문장, §8/§9/§10) 변경분에 대해 결함 2건(1차: §8 "코드 미반영 상태" 명시 누락, 2차: 카운터 공유 트레이드오프 — 문서 수정 없이 근거로 판정)을 모두 조치/판정 완료. 최소 2회 검증 하한 충족, 2차에서 새로운 Critical/High 결함이 발견되지 않아 3차 없이 종료. **5단계(WU-09 재작업, `core/admin_auth.py`/`config/urls.py` 코드 반영)로 handoff 가능. 이후 6→7→9단계 재실행이 완료되어야 10단계(배포) 재개가 가능함을 재확인(DEC-039/§8 항목 10).**
- [ ] FAIL

## 규칙A 재확인 (재작업 라운드 2)
이번 재작업 과정에서 사용자 확인이 필요한 두 갈래 이상의 미해결 쟁점은 발견되지 않았다. 옵션 1(라우트 제거) vs 옵션 2(양쪽 방어)는 "확신이 없어 질문해야 하는 두 갈래"가 아니라 §5.6(이미 PASS된 상위 설계 요구사항)과의 코드 대조로 한쪽이 명백히 기각되는 결정이었으므로 규칙A-3(비가역적 결정, 근거 충분)에 따라 에이전트 자체판단으로 확정하고 DEC-040에 기록했다. 사용자에게 별도로 전달할 질문 목록은 없다.

## 절차 흐름 (참고용 다이어그램, 재작업 라운드 2)
```mermaid
flowchart TD
    A["1차 검증: 작성자 관점"] --> B{결함 발견?}
    B -->|Yes: §8 코드미반영 상태 명시 누락| C["조치 → v1.3(1차분)"]
    C --> D["2차 검증: 9단계 보안검증자 관점"]
    D --> E{결함 발견?}
    E -->|Yes: 카운터 공유 트레이드오프 미서술, 근거로 판정| F["조치/판정 완료 → v1.3(최종)"]
    F --> G{"신규 Critical/High?"}
    G -->|No| H["PASS: 총 2회 검증 완료<br/>5단계(WU-09 재작업)로 handoff"]
```

---

# 재작업 라운드 3 — 규칙F(피드백 루프) 대응, 10단계 배포테스트 CONDITIONAL PASS 조건1(DEF-10-01) 근본원인 대응

> 아래는 기존 검증 로그(라운드 1/2의 v1.2/v1.3 PASS)를 대체하지 않고 append한다. 대상 변경분은 **`03-system-design.md` §5.5.2-1(신설) + §4/§8/§9의 상호참조 1줄씩**이며, v1.3 → v1.4로 버전업했다. §1~§4 본문, §5.1~§5.5.4(§5.5.2-1 제외), §6~§7 등 나머지 섹션은 전혀 건드리지 않았다. 트리거는 `docs/harness/10-deploy-test.md`(10단계, CONDITIONAL PASS, DEF-10-01 High)이며, 규칙F-1(근본 원인 단계까지 소급)에 따라 3단계로 되돌아와 이번 변경분에 대해 내부검증(규칙B) 최소 2회를 재수행한다.

## 1차 검증 (작성자 관점 자가 재검토 — 이번 재작업분)
- 검증자(역할): 03-system-designer (작성자 본인, 자가 재검토)
- 일시: 2026-09-25
- 체크리스트
  - [x] `10-deploy-test.md`가 지목한 근본 원인(§4의 healthz 계약과 §5.5.2의 HTTPS 강제가 서로를 참조하지 않은 설계 공백)이 실제로 해소되었는가 — §5.5.2-1 신설 + §4 `/healthz` 행에 상호참조 추가로 확인
  - [x] 이번 문서 변경이 "코드를 사후 추인"하는 것에 그치지 않고, 코드가 실제로 무엇을 하는지와 정확히 일치하는가 — `webapp/config/settings/production.py` 77행(`SECURE_REDIRECT_EXEMPT = [r"^healthz$"]`)을 직접 열람해 §5.5.2-1의 코드 블록과 문자 그대로 대조, 일치 확인. 정규식이 `^healthz$`로 다른 경로에 영향을 주지 않음을 재확인(과도한 예외 범위가 아님)
  - [x] 이 설계 결정이 새로운 보안 리스크를 만들지 않는가 — `/healthz`는 인증 없는 고정 "ok" 텍스트만 반환하고(§4), 세션/CSRF/민감정보를 전혀 다루지 않으므로 HTTPS 강제 예외로 인한 손실이 없음을 재확인(09단계 보안검증 SEC 계열 항목과 비교해도 새로운 공격면이 아님)
  - [x] §8/§9/§10/헤더가 실제 변경과 일치하는가 — §8 항목 11(Resolved) 신규, §9 REQ-011 매핑 갱신, §10 v1.4 행 추가, 헤더 입력목록/버전 갱신 — 전부 확인
  - [x] 규칙G(decisions.md)에 이번 결정을 기록했는가 — DEC-044로 기록 확인(WU-01/WU-10 두 유닛에 걸친 재작업을 통합 기록, 비가역성 근거 포함)
- 발견된 결함 목록: 없음
- 조치 내용: 해당 없음(결함 0건)

## 2차 검증 (독립 심사자 관점 — "10단계 배포테스터가 이 §5.5.2-1(v1.4)만 보고 TC-004를 재확인할 수 있는가")
- 검증자(역할): (가상) 10단계 배포테스터 — DEF-10-01을 실측 재현했던 바로 그 관점
- 일시: 2026-09-25
- 체크리스트 및 재검토 결과
  - [x] TC-004(헤더 없이 `/healthz` 요청)를 재실행했을 때 기대 결과(200)가 이 문서만으로 명확한가 — §5.5.2-1이 코드 블록과 함께 "헤더 없이도 200"을 명시적으로 서술, 명확함
  - [x] 예외가 `/healthz` 하나로 정확히 스코프되어 다른 보안 헤더/경로 검증(TC-005 등 10종 스모크)에 영향을 주지 않는가 — 정규식 `^healthz$` 검토 및 05단계(WU-01) 라운드2 회귀 테스트(비예외 경로 `/robots.txt`가 여전히 301)로 대조 확인됨을 §5.5.2-1·unit-01-note.md §4-2 인용으로 재확인
  - [x] "코드가 이미 되어 있었다"는 사실이 10단계 판정에 미치는 영향을 명확히 정리했는가 — 08/09단계가 검증한 코드와 10단계가 TC-004를 실행한 코드가 동일 커밋(`bcc17a2d7`, 2026-09-18) 기준이므로, 8/9단계를 다시 돌 필요 없이 **10단계 §9 CONDITIONAL PASS 조건 1번만 addendum으로 해소 처리**하면 된다는 점이 §10 변경이력에 명시됨을 확인 — 8/9 재실행 생략이 "이미 지나간 단계라 안 돌려도 된다"는 임의 판단이 아니라 "검증 대상 코드가 애초에 동일했다"는 사실에 근거함을 재확인(규칙F-3 예외 아님, 애초에 회귀 대상이 아님)
  - [x] 새로운 Critical/High 결함이 발견되는가 — 없음
- 발견된 결함 목록: 없음
- 조치 내용: 해당 없음(결함 0건, 1차와 동일)

## 최종 판정 (재작업 라운드 3)
- [x] **PASS** — §5.5.2-1(및 연계된 §4/§8/§9/§10) 변경분에 대해 1차·2차 모두 결함 0건. Tier=Standard 원문(최소 2회 고정)에 따라 1차 결함 0건이어도 2차를 생략하지 않고 수행했으며, 2차에서도 결함이 나오지 않아 3차 없이 종료. **10단계(`10-deploy-test.md`) addendum으로 handoff 가능 — CONDITIONAL PASS 조건 1번(DEF-10-01) 해소 근거로 이 문서(v1.4)와 `unit-01-note.md` §4-2를 인용한다.**
- [ ] FAIL

## 규칙A 재확인 (재작업 라운드 3)
이번 재작업 과정에서 사용자 확인이 필요한 두 갈래 이상의 미해결 쟁점은 발견되지 않았다. "코드는 이미 있었는데 문서만 갱신한다"는 처리 방식 자체가 비가역적이거나 해석이 갈리는 결정이 아니라(코드를 직접 열람해 사실관계를 확인한 뒤의 기록 정합 작업), 규칙A 질문 대상이 아니라고 판단했다. 사용자에게 별도로 전달할 질문 목록은 없다.

## 절차 흐름 (참고용 다이어그램, 재작업 라운드 3)
```mermaid
flowchart TD
    A["1차 검증: 작성자 관점"] --> B{결함 발견?}
    B -->|No| C["2차 검증: 10단계 배포테스터 관점"]
    C --> D{결함 발견?}
    D -->|No| E["PASS: 총 2회 검증 완료(결함 0건)<br/>10단계 addendum으로 handoff"]
```

---

# 재작업 라운드 4 — 신규 기능(REQ-019 댓글), 2026-09-25

> 대상 변경분: `03-system-design.md` §3.1(ERD COMMENT 신설) / §3.2-1(신설) / §4(엔드포인트 2건 추가) / §9 REQ-019 / §10. 나머지 섹션은 v1.4와 완전히 동일. 트리거는 사용자의 명시적 기능 추가 요청(신규 개발, 규칙F 대상 아님 — 결함 수정이 아니라 신규 범위 승격이므로 "재작업 라운드"라는 명칭을 쓰되 규칙F 프로세스가 아닌 통상 3단계 신규 설계 절차다).

## 1차 검증 (작성자 관점)
- 검증자: 03-system-designer(자가 재검토)
- 일시: 2026-09-25
- 체크리스트
  - [x] 사용자가 확정한 두 결정(사전승인제, 표시명만 수집)이 §3.2-1에 정확히 반영되었는가 — 반영 확인
  - [x] 회원가입이 없다는 기존 전제(DEC-009)와 모순 없이 설계되었는가 — `author_name` 자유입력, 계정 불필요 명시
  - [x] 방문자 입력(댓글 본문)이 이스케이프 없이 렌더링되는 경로가 설계에 남아있지 않은가 — plain TextField + auto-escape+linebreaks만 명시, `mark_safe`/`|safe` 금지 명시
  - [x] 기존 `subscribers` 앱의 스팸방지/IP마스킹 패턴을 재사용하도록 설계했는가(불필요한 재설계 방지) — 명시됨
  - [x] §8/§9/§10/헤더가 실제 변경과 일치하는가 — 확인
- 발견된 결함: 없음

## 2차 검증 (독립 심사자 관점 — "이 §3.2-1만 보고 5단계 개발자가 추가 질문 없이 구현을 시작할 수 있는가")
- 일시: 2026-09-25
- 체크리스트
  - [x] 모더레이션 권한(누가 승인하는가)이 구체적인가 — "Moderators 그룹(및 superuser)", Category 권한 부여 패턴과 동일하게 명시됨
  - [x] 앱 경계(어느 앱에 무엇을 넣는가)가 모호하지 않은가 — `comments` 신규 앱, `blog`는 자리만 포함한다고 명시
  - [x] 레이트리밋 임계값이 "확인 필요"로 막연히 남지 않고 구체적 초기값(10분/5회)과 조정 근거가 있는가 — 명시됨
  - [x] 삭제/수정 권한의 세부(원문 수정 가능 여부)를 5단계로 위임한 것이 무책임한 공백이 아니라 의도적으로 "확정할 필요 없는 세부"로 분류된 것인가 — §3.2-1 마지막 문장에서 근거와 함께 명시적으로 위임 처리됨(임의 누락 아님)
  - [x] 새로운 Critical/High 보안 결함 소지가 있는가(XSS/스팸/개인정보) — 없음, 기존 원칙(§5.2/§5.3/§5.6)을 그대로 확장 적용
- 발견된 결함: 없음

## 최종 판정 (재작업 라운드 4)
- [x] **PASS** — 결함 0건, 1차·2차 모두 통과(Standard 원문 2회 검증 충족). **5단계(신규 `comments` 앱 구현)로 handoff 가능.**
- [ ] FAIL

## 규칙A 재확인 (재작업 라운드 4)
사용자에게 이미 질문해 확정받은 두 가지(모더레이션 방식, 수집 항목) 외에 추가로 두 갈래 이상으로 해석이 갈리는 미해결 쟁점은 없다. 답글(threading) 미지원, 원문 수정 권한 미확정은 "질문이 필요한 두 갈래"가 아니라 과설계 방지 원칙에 따라 자체판단으로 확정/위임한 것(근거 §3.2-1 명시)이라 규칙A 대상이 아니다.
